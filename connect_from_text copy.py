...
def main():
    ...
    line_count = sum(1 for _ in pass_file)
    pass_file.seek(0)  # ファイルの先頭に移動
    ...
    while True:
        ...
        if connect_to_access_point(selected, password):
            print(f"{selected['ssid']} に接続しました{password}")
            return

        # print(f"{selected['ssid']} に接続できませんでした。別の番号を選択してください。")
        print(f"{selected['ssid']}に接続中…{counter/line_count*100:.1f}%")
        counter+=1
    print("")
    pass_file.close()
    return
