...
# 使用ライブラリ: pathlib（ファイル操作）、wifi_connection（JSON読み込みと接続処理）。
import argparse
import time
...
def try_candidate_passwords(
    ...
    for password in password_list:
        ...
        if success:
            return True, attempted, attempt_durations, time.perf_counter() - started_at
        attempted.append(password)
    return False, attempted, attempt_durations, time.perf_counter() - started_at
...
def main():
    ...
    while True:
        ...
        password_list = list(password_stream)
